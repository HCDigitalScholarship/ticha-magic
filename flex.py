"""Load and insert FLEx linguistic information into an HTML document. The main function of this
   module is flexify, which will insert FLEx annotations into properly formatted HTML data.

   For each Zapotec word or phrase (indicated by <mark> tags) in the document, the flexify function
   looks up the word in the FLEx dict (created by the load_flex module) and adds a popover HTML
   element with the FLEx data.
"""
import re
